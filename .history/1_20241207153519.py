...
def apply_one_hot_encoding(df, column_name, categories):
    for category in categories:
        # Преобразуем строку в нижний регистр и проверяем наличие категории
        df[category] = df[column_name].apply(lambda x: category.lower() in str(x).lower())
    return df
...
# Фильтруем только тех, у кого и проектная работа, и волонтёрство True
...
# Фильтруем только тех, у кого и вахтовый метод, и гибкий график True
