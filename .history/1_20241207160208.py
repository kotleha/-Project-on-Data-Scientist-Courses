...
# Преобразуем столбцы с датами в формат datetime и выделим только дату
df_resume['Обновление резюме'] = pd.to_datetime(df_resume['Обновление резюме'], format='%d.%m.%Y').dt.date
...
df_merged = df_merged.drop(columns=['ЗП', 'ЗП_сумма', 'Валюта', 'Обновление резюме', 'currency', 'proportion'])

# Результат
print(df_merged[['ЗП (руб)']])
